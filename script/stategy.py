...
#        f.close()
        ...
        # 判断当前点回家效益是否更优
        if self.Targets[0]['job']!=self.own_home and self.own_n_jobs > 7:
            self.canIGoHome()
...
#            new_benefit = np.sqrt((self.own_value)/(self.home_dist) * \
#                          sum([self.map[target['job']] for target in self.Targets])/ \
#                          (sum([target['step'] for target in self.Targets])+self.own_env.dist[self.Targets[-1]['job']][self.own_home]))
...
#            self.Targets = Targets[Benefit.index(maxBenefit)]
    ...
    def testAssess2(self):
        # 获取地图获取level个包裹的所有走法和步数
        Targets = self.getAllTargets(self.jobs, self.own_cur_pos, self.max_step-self.step, level = min(self.level, 10-self.own_n_jobs))
